...
class UIComponents:
    ...
    @staticmethod
    def display_results_table(predictions, classes, model_type):
                ...
                with st.spinner("Processing..."):
                    ...
                    if predictions is not None:
                        ...
                        fig = self.visualizer.plot_predictions(
                            image, ground_truth, predictions, classes, title=settings['model_type']
                        )
                        st.pyplot(fig)

                        df = self.ui.display_results_table(predictions, classes, settings['model_type'])
                ...
                with st.spinner("Running all selected models..."):
                    for i, model_name in enumerate(settings['models_to_compare']):
                        with cols[i]:
                            ...
                            if predictions is not None:
                                fig = self.visualizer.plot_predictions(
                                    image, ground_truth, predictions, classes, title=f"{model_name} Predictions"
                                )
                                st.pyplot(fig)
                                self.ui.display_results_table(predictions, classes, model_name)
                            else:
